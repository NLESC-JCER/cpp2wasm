...
For the Python based root finding web service, connexion was used as the web framework as it maps each path+method combination to a Python function and will handle the validation and serialization.
...
The request and response schemas are JSON schemas.

The operation identifier (operationId) in the specification gets translated by Connexion to a Python method that will be called when the path is requested. Connexion call the functin with the JSON parsed POST request body.
